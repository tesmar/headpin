...
class HttpResource
  ...
  class << self
    ...
    def get(a_path, headers={})
      ...
      Rails.logger.debug "Resource GET request headers: #{headers}"
      resource_permissions.before_get_callback(a_path, headers)
      client = rest_client(Net::HTTP::Get, :get, a_path, headers)
    ...
    def rest_client(http_type, method, path, headers= nil)
      ...
      request = http_type.new(url, headers)
